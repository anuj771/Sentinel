...
class CircularWheelView : ConstraintLayout {
     ...
     */
     val FLING_VELOCITY_DOWNSCALE = 20
    ...
    private fun getAttributedValues(attrs: AttributeSet) {
        ...
        normalColor = a.getColor(R.styleable.CircularWheelView_wheel_item_text_color, Color.BLACK)
        selectionColor = a.getColor(R.styleable.CircularWheelView_wheel_item_selected_text_color, Color.BLACK)
                ...
                if (wheelLayout.measuredHeight > 1) {
                    ...
                    with(param) {
                        ...
                        if (this@CircularWheelView.measuredWidth > width / 2) {
                            if (viewType == LEFT)
                                marginEnd = (this@CircularWheelView.measuredWidth - width * 1.8).toInt()
                            else
                                marginStart = (this@CircularWheelView.measuredWidth - width * 1.8).toInt()
                        }
                    }
                    ...
                    DEFAULT_IN_BETWEEN_SPACE = (param.width * 0.10f).toInt() // wheel font space
    ...
    private fun setPieRotation(rotation: Float) {
        ...
        if (gap < 0 && maxElementsCount != itemList.size) {
